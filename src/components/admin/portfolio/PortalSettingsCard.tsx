...
const PORTAL_TYPES: { id: PortalType; label: string }[] = [
  { id: 'production', label: 'Production' },
  { id: 'operations', label: 'Operations' },
  { id: 'food_beverage', label: 'Food & Beverage' },
  { id: 'talent', label: 'Talent & Industry' },
...
export function PortalSettingsCard({ projectId }: { projectId: string }) {
          ...
          <>
            ...
            <div className="flex justify-end pt-4">
              <Button onClick={handleSave} disabled={isSaving} className="w-full sm:w-auto">
                {isSaving ? 'Saving...' : `Save ${t.label} Portal`}
              </Button>
            </div>
  ...
  return (
    <div className="bg-white border rounded-lg shadow-sm p-6 lg:col-span-2">
      ...
      <Tabs tabs={tabs} activeTab={activeTab} onChange={(id) => setActiveTab(id as PortalType)} />
    </div>
  );
}
