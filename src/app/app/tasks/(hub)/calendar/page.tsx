...
function priorityColor(priority: string): string {
  return PRIORITY_COLORS[priority] ?? 'bg-bg-secondary text-text-muted border-border';
}
...
export default async function TaskCalendarPage({
  ...
  return (
    <TierGate feature="tasks">
      <PageHeader
        title="Task Calendar"
        subtitle="View tasks by due date on a calendar."
      />
      ...
      <div className="mt-4 flex flex-wrap gap-4 text-xs text-text-secondary">
        ...
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded border border-border bg-bg-secondary" />
          Low
        </div>
      </div>
    </TierGate>
