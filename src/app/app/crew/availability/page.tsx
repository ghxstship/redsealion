'use client';

import Link from 'next/link';
import { useEffect, useState } from 'react';

interface CrewAvailability {
  id: string;
  full_name: string;
  availability: Record<string, 'available' | 'unavailable' | 'tentative'>;
}

const CELL_COLORS: Record<string, string> = {
  available: 'bg-green-200',
  unavailable: 'bg-red-200',
  tentative: 'bg-yellow-200',
};

function getDaysInMonth(year: number, month: number): number {
  return new Date(year, month + 1, 0).getDate();
}

function buildFallbackData(year: number, month: number): CrewAvailability[] {
  const daysInMonth = getDaysInMonth(year, month);
  const names = ['Alex Rivera', 'Jordan Lee', 'Sam Patel', 'Morgan Chen', 'Taylor Brooks'];
  const statuses: Array<'available' | 'unavailable' | 'tentative'> = ['available', 'unavailable', 'tentative'];

  return names.map((name, idx) => {
    const availability: Record<string, 'available' | 'unavailable' | 'tentative'> = {};
    for (let d = 1; d <= daysInMonth; d++) {
      const dateKey = `${year}-${String(month + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
      // Deterministic pseudo-random based on name index and day
      const seed = (idx * 31 + d) % 10;
      availability[dateKey] = seed < 6 ? 'available' : seed < 8 ? 'tentative' : 'unavailable';
    }
    return { id: `crew_${String(idx + 1).padStart(3, '0')}`, full_name: name, availability };
  });
}

export default function CrewAvailabilityPage() {
  const [currentDate, setCurrentDate] = useState(() => new Date());
  const year = currentDate.getFullYear();
  const month = currentDate.getMonth();
  const [crew, setCrew] = useState<CrewAvailability[]>(() => buildFallbackData(year, month));

  const daysInMonth = getDaysInMonth(year, month);
  const monthName = currentDate.toLocaleString('default', { month: 'long' });
  const days = Array.from({ length: daysInMonth }, (_, i) => i + 1);

  useEffect(() => {
    setCrew(buildFallbackData(year, month));
  }, [year, month]);

  function goToPrevMonth() {
    setCurrentDate(new Date(year, month - 1, 1));
  }

  function goToNextMonth() {
    setCurrentDate(new Date(year, month + 1, 1));
  }

  return (
    <>
      {/* Header */}
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-foreground">
            Team Availability
          </h1>
          <p className="mt-1 text-sm text-text-secondary">
            View crew availability across the month.
          </p>
        </div>
        <Link
          href="/app/crew"
          className="rounded-lg border border-border bg-white px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-bg-secondary"
        >
          Back to Directory
        </Link>
      </div>

      {/* Month navigation */}
      <div className="mb-6 flex items-center justify-between">
        <button
          onClick={goToPrevMonth}
          className="rounded-lg border border-border bg-white px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-bg-secondary"
        >
          &larr; Previous
        </button>
        <h2 className="text-lg font-semibold text-foreground">
          {monthName} {year}
        </h2>
        <button
          onClick={goToNextMonth}
          className="rounded-lg border border-border bg-white px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-bg-secondary"
        >
          Next &rarr;
        </button>
      </div>

      {/* Calendar grid */}
      <div className="rounded-xl border border-border bg-white overflow-hidden overflow-x-auto">
        <table className="w-full min-w-[800px]">
          <thead>
            <tr className="border-b border-border bg-bg-secondary">
              <th className="sticky left-0 bg-bg-secondary px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-text-muted min-w-[150px]">
                Crew Member
              </th>
              {days.map((d) => (
                <th
                  key={d}
                  className="px-1 py-3 text-center text-xs font-medium text-text-muted min-w-[32px]"
                >
                  {d}
                </th>
              ))}
            </tr>
          </thead>
          <tbody className="divide-y divide-border">
            {crew.map((member) => (
              <tr key={member.id} className="transition-colors hover:bg-bg-secondary/30">
                <td className="sticky left-0 bg-white px-4 py-2 text-sm font-medium text-foreground border-r border-border">
                  <Link href={`/app/crew/${member.id}`} className="hover:underline">
                    {member.full_name}
                  </Link>
                </td>
                {days.map((d) => {
                  const dateKey = `${year}-${String(month + 1).padStart(2, '0')}-${String(d).padStart(2, '0')}`;
                  const status = member.availability[dateKey] ?? 'available';
                  return (
                    <td key={d} className="px-1 py-2">
                      <div
                        className={`mx-auto h-5 w-5 rounded ${CELL_COLORS[status]}`}
                        title={`${member.full_name} - ${dateKey}: ${status}`}
                      />
                    </td>
                  );
                })}
              </tr>
            ))}
          </tbody>
        </table>
      </div>

      {/* Legend */}
      <div className="mt-4 flex flex-wrap gap-4 text-xs text-text-secondary">
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded bg-green-200" />
          Available
        </div>
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded bg-red-200" />
          Unavailable
        </div>
        <div className="flex items-center gap-1.5">
          <span className="inline-block h-3 w-3 rounded bg-yellow-200" />
          Tentative
        </div>
      </div>
    </>
  );
}
