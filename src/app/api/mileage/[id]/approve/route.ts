...
export async function POST(_request: NextRequest, context: RouteContext) {
  ...
  if (mileage.status !== 'pending') {
    return NextResponse.json({ error: `Cannot approve mileage entry with status '${mileage.status}'` }, { status: 400 });
  }
  ...
  if (error || !data) {
    return NextResponse.json({ error: 'Failed to approve mileage', details: error?.message }, { status: 500 });
  }

  logAuditAction({
    orgId: perm.organizationId,
    action: 'mileage.approved',
    entity: 'mileage_entries',
    entityId: id,
    metadata: { status: mileage.status },
  }).catch(() => {});
