...
export async function GET(_request: NextRequest, context: RouteContext) {
  ...
  if (error || !data) return NextResponse.json({ error: 'Interaction not found' }, { status: 404 });
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  ...
  const allowedFields = ['type', 'subject', 'body', 'occurred_at'];
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  ...
  const { error } = await supabase.from('client_interactions').delete().eq('id', id).eq('organization_id', perm.organizationId);
