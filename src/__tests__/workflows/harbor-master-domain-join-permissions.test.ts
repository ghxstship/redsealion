/**
 * Harbor Master — Domain Match (C01–C06), Join Requests (D01–D07),
...
const ROLES: Record<string, HierarchyRole> = {
  owner: { id: SYSTEM_ROLE_IDS.OWNER, name: 'owner', level: 10 },
  admin: { id: SYSTEM_ROLE_IDS.ADMIN, name: 'admin', level: 20 },
  manager: { id: SYSTEM_ROLE_IDS.MANAGER, name: 'manager', level: 30 },
  member: { id: SYSTEM_ROLE_IDS.MEMBER, name: 'member', level: 40 },
  viewer: { id: SYSTEM_ROLE_IDS.VIEWER, name: 'viewer', level: 50 },
  guest: { id: SYSTEM_ROLE_IDS.GUEST, name: 'guest', level: 60 },
};
...
describe('Harbor Master — Domain Match Tests (C01–C06)', () => {
...
describe('Harbor Master — Join Request Tests (D01–D07)', () => {
...
describe('Harbor Master — Permission & Hierarchy Tests (E01–E10)', () => {
  it('E01: Manager invites as Admin → hierarchy violation', () => {
    expect(canAssignRole(ROLES.manager.level, ROLES.admin.level)).toBe(false);
  });

  it('E02: Manager invites as Member → success', () => {
    expect(canAssignRole(ROLES.manager.level, ROLES.member.level)).toBe(true);
  });
  ...
  it('E04: Owner changes member to Admin → success', () => {
  ...
  it('E06: Admin removes lower-level Member → success', () => {
    expect(canRemoveMember(ROLES.admin.level, ROLES.member.level, false)).toBe(true);
  });

  it('E07: Custom role with unowned permission → privilege escalation', () => {
    const result = canCreateCustomRole(
      ROLES.manager.level,
      ROLES.member.level, // valid level
      ['invite:member', 'read:member', 'manage:team'],
      ['invite:member', 'manage:billing'], // manage:billing not owned by manager
    );
    expect(result.valid).toBe(false);
  ...
  it('E08: API key assigns above its own role → blocked', () => {
    const apiKeyRoleLevel = ROLES.manager.level;
